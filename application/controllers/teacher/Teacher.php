...
class Teacher extends CI_Controller {
    ...
    function gettimetable($id = NULL) {
        ...
        $teacher_id = $this->session->userdata['student']['teacher_id'];
            ...
            // $this->db->where("teacher_subjects.class_section_id","class_sections.id");
            $this->db->where("teacher_subjects.teacher_id",$teacher_id);
            $cs=$this->db->get("class_sections");
            $data["lists"]=$this->db->query("SELECT classes.class,sections.section, sessions.session, class_time.* FROM class_time JOIN class_sections ON class_time.class_id=class_sections.class_id JOIN teacher_subjects ON teacher_subjects.class_section_id=class_sections.id JOIN classes ON classes.id=class_sections.class_id JOIN sections ON sections.id=class_sections.section_id JOIN sessions ON sessions.id=class_time.academic_year where teacher_subjects.teacher_id='$teacher_id'");
