...
class Inventory extends Admin_Controller {
    ...
    function insert_data() {
        ...
        $data["itemlist"]=$this->Inventory_model->get_itemname();
        
        $this->load->view('layout/header', $data);
        $this->load->view('inventory/'.$form, $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    function edit_data() {
        ...
        $data["row"]=$this->Inventory_model->get_data($form,$id);
        $data["itemlist"]=$this->Inventory_model->get_itemname();
        $data["salelist"]=$this->db->get_where("sale_detail",array("sale_id"=>$id));
        $data["purchaselist"]=$this->db->get_where("purchase_details",array("purchase_id"=>$id));
        $this->load->view('layout/header', $data);
        $this->load->view('inventory/edit_'.$form.'_form', $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    function searchdata()
    {
        ...
        switch($form)
        {
            case "stock":
                ...
                $query=$this->db->query("SELECT * FROM stock WHERE name LIKE '%$name%'");
                break;
            case "saleperson":
                $name=$this->input->post("sale_person");
                $query=$this->db->query("SELECT * FROM sale WHERE sale_person LIKE '%$name%' OR title LIKE '%$name%' ");
                $form="sale";
                break;
            case "sale":
                $start_date=date("Y-m-d",strtotime($this->input->post("start_date")));
                $end_date=date("Y-m-d",strtotime($this->input->post("end_date")));
                $query=$this->db->query("SELECT * FROM sale WHERE date BETWEEN '$start_date' AND '$end_date'");
                $form="sale";
                break;
            case "purchaseperson":
                $person=$this->input->post("purchase_person"); 
                $query=$this->db->query("SELECT * FROM purchase WHERE purchase_person LIKE '%$person%' OR title LIKE '%$person%'");
                $form="purchase";
                break;
            case "purchase":
                $start_date=date("Y-m-d",strtotime($this->input->post("start_date")));
                $end_date=date("Y-m-d",strtotime($this->input->post("end_date")));
                $query=$this->db->query("SELECT * FROM purchase WHERE date BETWEEN '$start_date' AND '$end_date'");
                $form="purchase";
                break;
            case "descript":
                $des=$this->input->post("description"); 
                $query=$this->db->query("SELECT use_item.*, stock.name FROM use_item LEFT JOIN stock ON use_item.item_id=stock.id WHERE use_item.description LIKE '%$des%' OR stock.name LIKE '%$des%' ");
                $form="use_item";
                break;
            case "use_item":
                $start_date=date("Y-m-d",strtotime($this->input->post("start_date")));
                $end_date=date("Y-m-d",strtotime($this->input->post("end_date")));
                $query=$this->db->query("SELECT use_item.*,stock.name FROM use_item LEFT JOIN stock ON use_item.item_id=stock.id WHERE use_item.date BETWEEN '$start_date' AND '$end_date'");
                $form="use_item";
                break;
        ...
        if($query->num_rows()>=1)
            ...
            $this->load->view('inventory/'.$form.'_list', $data);
    ...
    function getPrice()
    {
        ...
        $qry=$this->db->get_where("stock",array("id"=>$item_id))->row_array();
        $result=json_encode($qry);
        echo $result;
    }
    ...
    {
        $item_id=$this->uri->segment(3); 
        $qry=$this->db->get_where("stock",array("id"=>$item_id))->row();
        $price=$qry->price;
        echo $price;
    }
    ...
    function sale()
    {
        ...
        $this->session->set_userdata('sub_menu', 'Inventory/sale');
        // $data["school"]=$this->Common_model->grab_school();
        $data["resultlist"] = $this->db->get("sale");
        $this->load->view('layout/header', $data);
        $this->load->view('inventory/sale', $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    function purchase()
    {
        $data["for"]=date("F");
        $this->session->set_userdata('top_menu', 'Inventory');
        $this->session->set_userdata('sub_menu', 'Inventory/purchase');
        // $data["school"]=$this->Common_model->grab_school();
        $data["resultlist"] = $this->db->get("purchase");
        $this->load->view('layout/header', $data);
    ...
    function use_item()
    {
        $data["for"]=date("F");
        $this->session->set_userdata('top_menu', 'Inventory');
        $this->session->set_userdata('sub_menu', 'Inventory/use_item');
        // $data["school"]=$this->Common_model->grab_school();
        $this->db->select("use_item.*,stock.name");
        $this->db->join("stock","stock.id=use_item.item_id","left");
        $data["resultlist"] = $this->db->get("use_item");
        $this->load->view('layout/header', $data);
        $this->load->view('inventory/use_item', $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    function stock()
    {
        $data["for"]=date("F");
        $this->session->set_userdata('top_menu', 'Inventory');
        $this->session->set_userdata('sub_menu', 'Inventory/stock');
        // $data["school"]=$this->Common_model->grab_school();
        $data["resultlist"] = $this->db->get("stock");
        // $data["totalstock"]=$this->db->query("SELECT SUM() as total FROM stock")->row();
        $this->load->view('layout/header', $data);
    ...
    function create_stock() {
        ...
        if($query)
        {
            ...
            redirect(base_url()."Inventory/insert_data/stock_form");
        }
        else
        {
            $err= $this->db->error();
    ...
    function update_stock()
    {
        ...
        if($query)
        {
            ...
            redirect(base_url()."Inventory/edit_data/stock/".$id);
        }
        else
        {
            $err= $this->db->error();
    ...
    function create_purchase()
    {
        ...
        $purchase_person=$this->input->post("purchase_person");
        ...
        $qul=$this->db->insert("purchase",$hdata);
        ...
        if($qul)
        {
            for($i=0;$i<$count;$i++)
            {
                ...
                $query=$this->db->insert("purchase_details",$data);
                if($query)
                {
                    $qry=$this->db->query("Update stock SET quantity=quantity+'$quantity[$i]' Where id='$item_id[$i]'");
                }
            }
            ...
            {
                $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">Successfully Saved</div>');
                redirect(base_url()."Inventory/insert_data/purchase_form");
            }
            else
            {
                $err= $this->db->error();
    ...
    function update_purchase()
    {
        ...
        $this->db->where("id",$id);
        $query=$this->db->update("purchase",$hdata);
        ...
        if($query)
        {
            ...
            for($i=0;$i<$count;$i++)
            {
                ...
                if($query)
                {
                    $qry=$this->db->query("Update stock SET quantity=quantity+'$quantity[$i]'-'$old_qty' Where id='$item_id[$i]'");
                }
            }
                ...
                {
                    $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">Successfully Updated</div>');
                    redirect("Inventory/edit_data/purchase/".$id);
                }
        ...
        {
            $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">Failed to update</div>');
                redirect(base_url()."Inventory/edit_data/purchase/".$id);
        }
    }
    ...
    function create_sale()
    {
        ...
        if($qul)
        {
            for($i=0;$i<$count;$i++)
            {
                ...
                $query=$this->db->insert("sale_detail",$data);
                if($query)
                {
                    $qry=$this->db->query("Update stock SET quantity=quantity-'$quantity[$i]' Where id='$item_id[$i]'");
                }
            ...
            {
                $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">Successfully Saved</div>');
                redirect(base_url()."Inventory/insert_data/sale_form");
            }
            else
            {
                $err= $this->db->error();
    ...
    function update_sale()
    {
        ...
        $this->db->where("id",$id);
        $query=$this->db->update("sale",$hdata);
        ...
        if($query)
        {
            ...
            for($i=0;$i<$count;$i++)
            {
                ...
                if($query)
                {
                    $qry=$this->db->query("Update stock SET quantity=quantity+ '$old_qty' - '$quantity[$i]' Where id='$item_id[$i]'");
                }
            }
                ...
                {
                    $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">Successfully Updated</div>');
                    redirect("Inventory/edit_data/sale/".$id);
                }
        ...
        {
            $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">Failed to update</div>');
                redirect(base_url()."Inventory/edit_data/sale/".$id);
        }
    }
    ...
    function create_expense() {
        ...
        $query=$this->db->insert("use_item",$data);
        if($query)
        {
            $qry=$this->db->query("Update stock SET quantity=quantity-'$quantity[$i]' Where id='$item_id[$i]'");
        }
        ...
        {
            $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">Successfully Saved</div>');
            redirect(base_url()."Inventory/insert_data/use_item_form");
        }
        else
        {
            $err= $this->db->error();
    ...
    function update_expense()
    {
        ...
        $this->db->where("id",$id);
        $query=$this->db->update("use_item",$data);
       
        if($query)
        {
            $qry=$this->db->query("Update stock SET quantity=quantity+'$old_qty'-'$quantity' Where id='$item_id'");
        if($qry)
        {
            $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">Successfully Updated</div>');
            redirect("Inventory/edit_data/use_item/".$id);
        }
    ...
    function view() {
        ...
        {
            case "sale":
                $this->db->select("sale_detail.*,stock.name");
                $this->db->join("stock","stock.id=sale_detail.item_id");
                $data["lists"]=$this->db->get_where("sale_detail",array("sale_id"=>$id));
                break;
                
            case "purchase":
                $this->db->select("purchase_details.*,stock.name");
                $this->db->join("stock","stock.id=purchase_details.item_id");
                $data["lists"]=$this->db->get_where("purchase_details",array("purchase_id"=>$id));
                break;  
        }

        $this->load->view('layout/header', $data);
        $this->load->view('inventory/'.$form.'_detail', $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    function printvoc() {
        ...
        $this->load->view('inventory/'.$form.'_print', $data);
