...
class Balance_fee extends Admin_Controller {
    ...
    function index() {
        ...
        $this->session->set_userdata('sub_menu', 'balance_fee/index');
        $data['title'] = 'Balance fees';
        $data["school"]=$this->Common_model->grab_school();

        $resultlist = $this->Studentfee_model->getBalance(0);
        $data['resultlist'] =$resultlist;
        $class = $this->class_model->get();
        $data['classlist'] = $class;
        $this->load->view('layout/header', $data);
        $this->load->view('balancefee/balancefeeNoPaidlist', $data); 
    ...
    function create($id) {
        ...
        $mfdata=array("fee_type"=>$fee_type,
                      "payment_for"=>$month,
                      "feegroup_id"=>$feegroupid,
                      "created_date"=>$created_date
        );
        $qry=$this->db->insert("monthlyfee",$mfdata);
        if($qry)
        {
        ...
        // $row=$this->db->get_where("student_session",array("class_id"=>$class_id));
          if($fee_type=="By class")
          {
               for($j=0;$j<12;$j++)
               {
                ...
                if($this->input->post("month".$j) != "")
                {
                    ...
                    // $query=$this->db->query("SELECT student_session.*, students.id as sid,students.firstname,students.lastname FROM student_session JOIN students ON student_session.student_id=students.id WHERE NOT EXISTS (SELECT * FROM install_student WHERE student_session.student_id = install_student.student_id AND install_student.session_id='$session_id') AND student_session.class_id='$class_id' AND student_session.session_id='$session_id'");
                ...
                //   echo $query->num_rows(); exit;
                    ...
                    $query=$this->db->insert("balance_fee",$data);          
          ...
          else if($fee_type=="By student")
          {
            ...
            $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">Successfully added</div>');
           redirect(base_url()."Studentfee/monthlyfee");
    ...
    function delete_monthlyfee()
    {
        $id=$this->uri->segment(3); 
        $qrl=$this->db->delete('monthlyfee', array('id' => $id)); 
        if($qrl)
        {
            $this->db->delete('balance_fee', array('mid' => $id)); 
        }
        redirect('studentfee/monthlyfee');
    }
    ...
    function viewbalance($id)
    {
        ...
        $data['title'] = 'student fees';
        $data["school"]=$this->Common_model->grab_school();
        $resultlist = $this->Studentfee_model->getBalanceById($id);
        $data['resultlist'] =$resultlist;
        $class = $this->class_model->get();
        $data['classlist'] = $class;
        $this->load->view('layout/header', $data);
  ...
  function delete($id) {
    ...
    function searchStudent()
    {
        $session_id=$this->setting_model->getCurrentSession();
        $class_id=$this->input->post("class_id");
        // echo $session_id; echo $class_id; exit;
        
       $query=$this->db->query("SELECT student_session.*, students.id as sid,students.firstname,students.lastname FROM student_session JOIN students ON student_session.student_id=students.id WHERE NOT EXISTS (SELECT * FROM install_student WHERE student_session.student_id = install_student.student_id AND install_student.session_id='$session_id') AND student_session.class_id='$class_id' AND student_session.session_id='$session_id'");
        
    // 	$query=$this->db->query("SELECT students.* FROM students JOIN student_session ON students.id=student_session.student_id LEFT JOIN install_student ON install_student.student_id=students.id JOIN classes ON student_session.class_id=classes.id  WHERE install_student.student_id != students.id AND classes.id='$class_id' AND student_session.session_id='$session_id'");
	   // echo $this->db->last_query(); exit;
	   //$err=$this->db->error();
	   //echo $err["message"]; exit;
		foreach($query->result() as $row):
		$r.= "<option value='".$row->sid."'>".$row->firstname." ".$row->lastname."</option>";
		endforeach;
	    echo $r;
