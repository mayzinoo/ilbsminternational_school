...
class Income extends Admin_Controller {
    ...
    function index() {
        ...
        // $data["incometotal"]=$this->db->query("Select SUM(amount) as total FROM income")->row();
    ...
    function incomeSearch_list()
{
    $this->session->set_userdata('top_menu', 'Income');
        $this->session->set_userdata('sub_menu', 'income/incomeSearch_list');
        $data['title'] = 'Search Income';
        if ($this->input->server('REQUEST_METHOD') == "POST") {
            $search = $this->input->post('search');
            if ($search == "search_filter") {
                ...
                $date_from = date('Y-m-d', $this->customlib->datetostrtotime($this->input->post('date_from')));
                $date_to = date('Y-m-d', $this->customlib->datetostrtotime($this->input->post('date_to')));
                $resultList = $this->income_model->income_search("", $date_from, $date_to);
                $data['resultList'] = $resultList;
            } else {
                $data['inc_title'] = 'Income Result';
                $search_text = $this->input->post('search_text');
                $resultList = $this->income_model->income_search($search_text, "", "");
                $data['resultList'] = $resultList;
            }
            $this->load->view('layout/header', $data);
            $this->load->view('admin/income/incomeSearch_list', $data);
            $this->load->view('layout/footer', $data);
        } else {
            $this->load->view('layout/header', $data);
            $this->load->view('admin/income/incomeSearch_list', $data);
    ...
    function incomelist_search()
    {
    	...
    	{
    		$query=$this->db->query("SELECT * FROM income");
    		$data["total_amt"]=$this->db->query("SELECT SUM(amount) as incomeamt FROM income")->row();
    	}
    	elseif(!empty($startdate) && empty($enddate))
    	{
    		$query=$this->db->query("SELECT * FROM income WHERE date='$startdate'");
    		$data["total_amt"]=$this->db->query("SELECT SUM(amount) as incomeamt FROM income WHERE date='$startdate'")->row();
    	}
    	elseif(empty($startdate) && !empty($enddate))
    	{
    		$query=$this->db->query("SELECT * FROM income WHERE date='$enddate'");
    		$data["total_amt"]=$this->db->query("SELECT SUM(amount) as incomeamt FROM income WHERE date='$enddate'")->row();
    	}
    	elseif(!empty($startdate) && !empty($enddate))
    	{
    		$query=$this->db->query("SELECT * FROM income WHERE date BETWEEN '$startdate' AND '$enddate'");
    		$data["total_amt"]=$this->db->query("SELECT SUM(amount) as incomeamt FROM income WHERE date BETWEEN '$startdate' AND '$enddate'")->row();
    	}
		...
		$data["message"]="";
		$data["incomelists"]=$query;
	

		$this->load->view('layout/header', $data);
        $this->load->view('admin/income/income_list_search', $data);
        $this->load->view('layout/footer', $data);
		}
    }
