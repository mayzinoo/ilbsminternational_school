...
class admin extends Admin_Controller {
    ...
    function dashboard() {
        ...
        $totalfeepermonth=$this->admin_model->get_totalfeepermonth($year_str_month, $year_end_month);
        
        $get_totalinstallpermonth=$this->admin_model->get_totalinstallpermonth($year_str_month, $year_end_month);
        
        $get_totalexppermonth=$this->admin_model->get_totalexppermonth($year_str_month, $year_end_month);
        
        $get_totalincpermonth=$this->admin_model->get_totalincpermonth($year_str_month, $year_end_month);
        
        $get_totalsummer=$this->admin_model->get_totalsummer($year_str_month, $year_end_month);
        
        $get_totalsale=$this->admin_model->get_totalsale($year_str_month, $year_end_month);
    ...
    //   $getDepositeAmount = $this->studentfeemaster_model->getDepositAmountBetweenDate($year_str_month, $year_end_month);
        ...
        $data['totalfeepermonth'] = $totalfeepermonth;
        $data['get_totalinstallpermonth'] = $get_totalinstallpermonth;
        $data['get_totalexppermonth'] = $get_totalexppermonth;
        $data['get_totalincpermonth'] = $get_totalincpermonth;
        $data['get_totalsummer'] = $get_totalsummer;
        $data['get_totalsummer'] = $get_totalsummer;

        $data['month_collection'] = $month_collection;
        $data['get_totalsale'] = $get_totalsale;
    ...
    function feedback()
    {
        ...
        if($this->uri->segment(4))
        {
         ...
         $data['lists'] = $this->db->query("SELECT students.guardian_name,students.firstname,students.lastname ,feedback.* FROM feedback JOIN students ON feedback.user_id=students.id WHERE feedback.id=$id AND feedback.viewm=0 order by id DESC");
        $updata=array("viewm"=>1);
        $this->db->where("id",$id);
        $this->db->update("feedback",$updata);
        }
        else
        {
        $data['lists'] = $this->db->query("SELECT students.guardian_name,students.firstname,students.lastname ,feedback.* FROM feedback JOIN students ON feedback.user_id=students.id order by id DESC");
        }
    ...
    function mailreply()
    {
        ...
        if($this->uri->segment(4)=="website_message")
        {
            ...
            $replyby="admin";
            $this->email->from("admin@ilbsm.com",$replyby);
            $this->email->to($replyto);      
            $this->email->subject($subject);
            $this->email->message($message);
            $this->email->send(); 
            
                $data=array("reply_status"=>1,
                            "reply_message"=>$message);
                $this->db->where("id",$id);
                $this->db->update("message_from_website",$data);
                $data["message"]="Successfully Sent!";
                $data['lists'] = $this->db->get("message_from_website");
        }
        
        if($this->uri->segment(4)=="onlinecv")
        {
            $form="onlinecv";
            $id=$this->input->post("id");
            $replyto=$this->input->post("replyto");
            ...
            $this->email->send(); 
                ...
                $data=array("reply_status"=>1,
                            "reply_message"=>$message);
                $this->db->where("id",$id);
                $this->db->update("onlinecv",$data);
                $data["message"]="Successfully Sent!";
                $data['lists'] = $this->db->get("onlinecv");
        }
        
        if($this->uri->segment(4)=="feedback")
        {
            $form="feedback";
            ...
            $replyby="admin";
                ...
                $data["message"]="Successfully Sent!";
                $data['lists'] = $this->db->get("feedback");
        }
    ...
    function website_message()
    {
        ...
        {
         $id=$this->uri->segment(4);
         $data['lists'] = $this->db->get_where("message_from_website",array("id"=>$id));  
        $updata=array("status"=>1);
        $this->db->where("id",$id);
        $this->db->update("message_from_website",$updata);
        }
        else
        {
        $data['lists'] = $this->db->get("message_from_website");
        ...
        $this->load->view('layout/header', $data);
        $this->load->view('admin/website_message', $data);
        $this->load->view('layout/footer', $data);
    }
     ...
     function online_registration()
    {
        ...
        {
         $id=$this->uri->segment(4);
         $data['lists'] = $this->db->get_where("student_info",array("id"=>$id));
         $updata=array("status"=>1);
         $this->db->where("id",$id);
         $this->db->update("student_info",$updata);
        }
        else
        {
        $data['lists'] = $this->db->get("student_info");
        }
     ...
     function onlinecv()
    {
        ...
        {
         $id=$this->uri->segment(4);
         $data['lists'] = $this->db->get_where("onlinecv",array("id"=>$id));
         $updata=array("status"=>1);
         $this->db->where("id",$id);
         $this->db->update("onlinecv",$updata);
        }
        else
        {
        $data['lists'] = $this->db->get("onlinecv");
        ...
        $this->load->view('layout/header', $data);
        $this->load->view('admin/onlinecv', $data);
        $this->load->view('layout/footer', $data);
    }
