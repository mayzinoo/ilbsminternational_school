...
class Primary_Reportcard extends Admin_Controller {
...
//        $data["subject"] = $this->db->get_where("subjects",array("level"=>'G1'));
//        $data['month'] = $this->db->get_where("reportcard_month",array("name !="=>'ေမ'));
//        $data['primary_result']=$this->db->get_where("primary_result",array('student_id'=>$student_id));
       
       
// // echo $data['primaryresult']->reportcard_month;exit;
//         $this->load->view('admin/primaryreportcard/reportcardFrontPrint', $data);
        $student_id=$this->uri->segment(4);
        $setting_result = $this->Reportcard_model->reportcard_head();
        $data['settinglist'] = $setting_result;     
        $data["student"]=$this->student_model->get($student_id);
