...
class Middle_Reportcard extends CI_Controller {
    ...
    function activity() {
     ...
     $data["reportcard_month"]=$this->Common_model->grab_month();
        ...
        $this->load->view('parent/reportcard/activityList', $data);
     ...
     function index() {
        ...
        $data['studentlist'] = $student_result;
        $this->load->view('layout/parent/header', $data);
        $this->load->view('parent/reportcard/reportcardList', $data);
        $this->load->view('layout/parent/footer', $data);
            ...
            if (isset($search)) {
                if ($search == 'search_filter') {
                    ...
                    if ($this->form_validation->run() == FALSE) {
                        ...
                        $this->load->view('parent/reportcard/activityformList', $data);
    ...
    function rpcardsfront() {
       ...
       $data["exams"]=$this->db->order_by("id","ASC")->get_where("exams",array("level"=>"High"));
	   $data["reportcard_subject"]=$this->db->order_by("id","ASC")->get_where("reportcard_subject",array("level"=>"middle"));
		/*$data["rpcards"]=$this->db->query("SELECT exam_results.*,exam_schedules.* FROM exam_results JOIN exam_schedules
                            ON exam_results.exam_schedule_id=exam_schedules.id  WHERE 
                            exam_results.student_id='$id'");*/
							
			$this->load->view('layout/parent/header', $data);
	        $this->load->view('parent/reportcard/middlereportcardFrontwithGrades.php', $data);
			$this->load->view('layout/parent/footer', $data);
    ...
    function printwithgrade() {
	   ...
	   $data["reportcard_subject"]=$this->db->order_by("id","ASC")->get_where("reportcard_subject",array("level"=>"middle"));
        ...
        $this->load->view('parent/reportcard/middlereportcardFrontwithGrades', $data);
    }
	...
	function rpcardsback() {
       ...
       $data["school_activity"]=$this->db->order_by("id","ASC")->get("school_activity");
        ...
        $this->load->view('parent/reportcard/middlereportcardBack', $data);
    }
    ...
    function rpcardsbackwithgrades() {
       ...
       $data["school_activity"]=$this->db->order_by("id","ASC")->get("school_activity");
        ...
        $this->load->view('parent/reportcard/reportcardBackPrintwithGrades', $data);
    }
    ...
    function edit($id) {
        ...
        if ($this->form_validation->run() == FALSE) {
            $this->load->view('layout/parent/header', $data);
            $this->load->view('parent/exam/examEdit', $data);
            $this->load->view('layout/parent/footer', $data);
        } else {
            $data = array(
                'id' => $id,
    ...
    function examSearch() {
        ...
        if ($this->input->server('REQUEST_METHOD') == "POST") {
            ...
            $this->load->view('layout/parent/header', $data);
            $this->load->view('parent/exam/examSearch', $data);
            $this->load->view('layout/parent/footer', $data);
        } else {
            $this->load->view('layout/parent/header', $data);
            $this->load->view('parent/exam/examSearch', $data);
