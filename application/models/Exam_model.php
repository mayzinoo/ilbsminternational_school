...
class Exam_model extends CI_Model {
    ...
    public function remove($id) {
        $this->db->where('id', $id);
       $qry= $this->db->delete('inter_exam_schedules');

       if($qry)
       {
        $this->db->where('header_id', $id);
        $this->db->delete('inter_exam_schedules_details');
    ...
    function add_inter_exam_schedule($data) {
        $this->db->where('exam_id', $data['exam_id']);
        $this->db->where('subject_id', $data['subject_id']);
        $q = $this->db->get('inter_exam_schedules');
        
        if ($q->num_rows() > 0) {
            $result = $q->row_array();
            $this->db->where('id', $result['id']);
            $this->db->update('inter_exam_schedules', $data);
        } else {
            $this->db->insert('inter_exam_schedules', $data);
        }
    }
