...
class Course_model extends CI_Model {
    ...
    function grab_lecture_course()
    {
        $this->db->order_by("id");

				$query = $this->db->get("lectured_courses");
    ...
    {
        if($form=="courses_register")
        {
            $this->db->select("courses_register.*,lectured_courses.fees as fees,lectured_courses.start_date as start_date,lectured_courses.end_date as end_date"); 
            $this->db->join("lectured_courses","lectured_courses.id=courses_register.lecture_course_id",'left');
            $query=$this->db->get_where($form,array("courses_register.id"=>$id))->row();
            return $query;
        }
