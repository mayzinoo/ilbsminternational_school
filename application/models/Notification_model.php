...
LEFT JOIN read_notification ON send_notification.id = read_notification.notification_id and read_notification.parent_id=" . $this->db->escape($parentid) . " where send_notification.visible_parent='Yes' order by send_notification.publish_date desc");
 ...
 public function getPrivateNotificationForParent($parentid = null) {
        $date = date('Y-m-d');
        $query = $this->db->query("SELECT
messages.id,messages.title,messages.created_at,messages.message,
IF (read_messages.id IS NULL,'unread','read') as notification_id
FROM messages
LEFT JOIN read_messages ON messages.id = read_messages.notification_id and read_messages.parent_id=" . $this->db->escape($parentid) . " where FIND_IN_SET($parentid,user_list) order by messages.created_at desc");
        return $query->result_array();
    }
  ...
  public function countUnreadMessageParent($parentid = null) {
        $date = date('Y-m-d');
        $query = $this->db->query("select * from (SELECT  IF (read_messages.id IS NULL,'unread','read') as notification_id FROM messages LEFT JOIN read_messages ON messages.id = read_messages.notification_id and read_messages.parent_id=" . $this->db->escape($parentid) . " where  FIND_IN_SET($parentid,user_list)) final where notification_id ='unread'");
        return $query->num_rows();
    }

    
     public function countUnreadFeedbackParent($parentid = null) {
        $date = date('Y-m-d');
        $query = $this->db->query("SELECT * FROM feedback WHERE viewm=0 AND user_id='$parentid' AND reply_by != '' ");
        return $query->num_rows();
    }
 ...
 public function updateprivatemessagestatus($notification_id, $parentid) {
        $this->db->where('notification_id', $notification_id);
        $this->db->where('parent_id', $parentid);
        $q = $this->db->get('read_messages');
        if ($q->num_rows() > 0) {
            return true;
        } else {
            $data = array(
            ...
            $this->db->insert('read_messages', $data);
        }
    }
