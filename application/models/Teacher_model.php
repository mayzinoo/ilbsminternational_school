...
class Teacher_model extends CI_Model {
     ...
     * @return mixed
     */
    public function get($id=null,$location=null,$resign=0) {
...
*/
        if ($id != null) {
            ...
            $this->db->where('teachers.resign', $resign);

      

            $this->db->select("sch_settings.name as ltext ,teachers.*");
            $this->db->join('sch_settings',"teachers.location=sch_settings.id");
            $this->db->order_by('id');
        }
    ...
    public function getTotalTeacher() {
        $sql = "SELECT count(*) as `total_teacher` FROM `teachers` where resign=0";
