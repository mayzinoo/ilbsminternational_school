...
class Infocenter_model extends CI_Model {
    ...
    public function get_feedbacks($id = null) {
        
        $this->db->select('students.guardian_name ,feedback.*')->from('feedback');
        $this->db->join('students', 'feedback.user_id = students.id');
        $this->db->where('feedback.viewm', 0);
