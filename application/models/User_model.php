...
class User_model extends CI_Model {
    ...
    public function read_user_information($username) {
        $this->db->select('users.*,students.firstname,students.lastname,students.school,students.guardian_name,student_session.class_id,student_session.inter_class,student_session.section_id');
        $this->db->from('users');
        $this->db->join('students', 'students.id = users.user_id');
        $this->db->join('student_session','student_session.student_id=users.user_id');
    ...
    public function read_teacher_information($username) {
        $this->db->select('users.*,teachers.name,teachers.location as school');
        $this->db->from('users');
    ...
    public function read_sub_admin_information($username) {
    ...
    public function getTeacherLoginDetails($teacher_id) {
        ...
        $this->db->like('role', "teacher","after");
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result();
        } else {
