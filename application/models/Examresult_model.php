...
class Examresult_model extends CI_Model {
    ...
    function add_exam_result($data) {
     ...
     function add_inter_exam_result($data) {
        $this->db->where('exam_schedule_id', $data['exam_schedule_id']);
        $this->db->where('student_id', $data['student_id']);
        $q = $this->db->get('inter_exam_results');
        $result = $q->row();
        if ($q->num_rows() > 0) {
            $this->db->where('id', $result->id);
            $this->db->update('inter_exam_results', $data);
            if ($result->get_marks != $data['get_marks']) {
                return $result->id;
            }
        } else {
            $this->db->insert('inter_exam_results', $data);
    ...
    public function get_result($exam_schedule_id = null, $student_id = null) {
     ...
     public function get_inter_result($exam_schedule_id = null,$subject_id = null, $student_id = null) {
        $this->db->select()->from('inter_exam_results');
        $this->db->where('exam_schedule_id', $exam_schedule_id);
        $this->db->where('student_id', $student_id);
