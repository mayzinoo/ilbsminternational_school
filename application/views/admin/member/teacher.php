		...
		<div class="download_label"><?php echo $this->lang->line('teacher_list'); ?></div>
            <table class="table table-striped table-bordered table-hover example">
                ...
                <tbody>
                    ...
                    <td id="status<?=$teacher['id']?>"><?php echo $library_card_no; ?></td>
                                ...
                                <td class="text text-right">
                                    ...
                                    <div class="modal fade" id="myModal<?php echo $teacher['id']?>" role="dialog">
<div class="modal-dialog modal-sm">
        <?=form_open("Member/member_edit/", "id='form".$teacher['id']."'")?>
        <?=form_hidden("id",$member_id);?>
      <div class="modal-content">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal">&times;</button>
          <h4 class="modal-title">Change Member Card No</h4>
        </div>
        <div class="modal-body"> Card No
            <?=form_input("library_card_no",$library_card_no,"class='form-control'")?>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-default" data-dismiss="modal" onclick="member_edit('<?=$teacher["id"]?>')">Save</button>
        </div>
      </div>
      <?=form_close()?>
    </div>
     ...
     <a href="#" class="btn btn-default btn-xs"  data-toggle="modal" data-target="#myModal<?php echo $teacher['id']?>" title="<?php echo $this->lang->line('edit'); ?>">
             <i class="fa fa-pencil"></i>
         </a>
     <a href="<?php echo base_url(); ?>admin/Member/member_delete/<?php echo $teacher["id"]?>"class="btn btn-default btn-xs"  data-toggle="tooltip" title="<?php echo $this->lang->line('delete'); ?>" onclick="return confirm('Are you sure you want to delete this item?')";>
...
function member_edit(id)
                {
                    data=$("#form"+id).serialize();
                            $.ajax({
                                    type:"POST",
                                    data: data,
                                    url:'<?php echo base_url() ?>admin/Member/member_edit',
                                    cache: false,
                                    success: function(d) 
